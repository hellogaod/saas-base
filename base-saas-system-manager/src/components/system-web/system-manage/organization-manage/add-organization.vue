<template>
  <section class="component add-organization" @click="treeShow=false">
    <el-form :model="addModel" :rules="rules" ref="add-form" label-width="90px">
      ...
      <el-form-item label="上级组织" prop="parentOrgName" :rules="addModel.parentOrgName ? {required: false} : {required: true, message: '请选择上级组织', trigger: 'blur'}">
        ...
        <div class="tree-wrapper" v-show="treeShow">
          <el-tree ref="add_tree" :data="orgList" :props="defaultProps" @node-click="handleNodeClick" :expand-on-click-node=false :filter-node-method="filterNode"></el-tree>
        </div>
      </el-form-item>
