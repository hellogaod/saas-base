<template>
  <el-table
    ...
    <el-table-column v-for="(column, index) in columns" :key="column.dataIndex" :align="index === 0 ? 'left' : 'center'" header-align="center"
      :label="column.text">
      <template slot-scope="scope">
        ...
        {{columns[index].dataIndex == 'orgType' || columns[index].dataIndex == 'status' ? '' : scope.row[column.dataIndex]}}
        ...
        <el-button v-if="columns[index].dataIndex == 'operation' && columns[index].btns.length > 0" v-for="(btnItem, btnIndex) in columns[index].btns" :key="btnIndex"
          size="mini"
          :type=btnItem.type
          :auth=btnItem.auth
...
import { DataTransfer } from "~/utils/dataTransfer.service";
