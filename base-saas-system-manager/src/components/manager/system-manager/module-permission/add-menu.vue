<template>
    <section class="component add-menu moduleAdd">
        <el-form :model="addModel" :rules="rules" ref="add-form" label-width="90px">
            <el-form-item label="父级菜单:" align="left" prop="parentName" v-show="parentMenu">
                <el-select  class="inpurClass"  v-model="addModel.id" @change="changeParentName">    
                    <el-option v-for="(item,index) in addModel.options" :key="index" :label="item.menuName" :value="item.id">{{item.menuName}}</el-option>
                </el-select>
            </el-form-item>
            ...
            <el-form-item label="菜单序列" prop="sequence">
                <el-input v-model="addModel.sequence" name="sequence" oninput="if(value.length>3)value=value.slice(0,3)" onkeyup="this.value=this.value.replace(/\s+/g,'')" type="number" min="1"></el-input>
            </el-form-item>
        ...
        <el-dialog title="选择图标" :append-to-body='true' class="iconDialog" v-model="addModel.icon" :center="true" :visible.sync="dialog.iconDialog" width="700px">
