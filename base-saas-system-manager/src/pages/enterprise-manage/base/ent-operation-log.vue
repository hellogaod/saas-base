<template>
  ...
  <div>
    <section class="page system-operationLog">
      <data-form :model="systemLogModel" @onSearch="refreshData">
        ...
        <template slot="collapse-input">
          <el-form-item label="操作时间:" prop="promotionTime" class="noticePicker">
            <el-date-picker value-format="yyyy-MM-dd" v-model="systemLogModel.promotionTime" type="daterange"
      ...
      <data-box :data="systemLogDataSet" :page="pageUtil" @onPageChange="refreshData">
        <template slot="columns">
          ...
          <el-table-column prop="exceptionCode" label="异常编码" :min-width="$helper.getColumnWidth(2)"
    ...
    <!--详情弹出框-->
    <el-dialog title="日志详情" :center="true" :visible.sync="dialog.detalis" width="800px"
               @open="$nextTick(()=>$refs['detalis-logs'].refresh(logObj))">
...
<script lang="ts">
  ...
  import {EntLogService} from "~/server/services/enterprise-manage-services/ent-log.service";
  import {PageUtil} from "~/utils/page.util";
  import DataForm from "~/components/common/data-form.vue";
  import DataBox from "~/components/common/data-box.vue";
  import systemView from "~/pages/enterprise-manage/base/ent-operation-log-detail.vue";
  ...
  export default class GoodsManage extends Vue {
    @Dependencies(EntLogService) private EntLogService: EntLogService;
