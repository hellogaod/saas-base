<template>
  ...
  <div>
    <section class="page system-modular">
      ...
      <data-box :data="modulDataSet" :page="pageService" @onPageChange="refreshData">
        <template slot="columns">
          ...
          <el-table-column label="操作" :min-width="$helper.getColumnWidth(5)">
            <template slot-scope="scope">
              ...
              <el-button type="text" @click="goToPage('/system-manage/modular-permission',scope.row.sysCode),modulObj = scope.row">业务功能</el-button>
            </template>
          </el-table-column>
        </template>
      </data-box>
      <!--添加模块-->
      <el-dialog title="添加模块" :center="true" :visible.sync="dialog.add" width="600px" @open="$nextTick(()=>$refs['add-modular'].reset())">
        <add-modular @close="dialog.add = false" ref="add-modular"  @refreshList="refreshData"></add-modular>
      </el-dialog>
      <!--编辑模块-->
      <el-dialog title="编辑模块" :center="true" :visible.sync="dialog.modify" width="600px" @open="$nextTick(()=>$refs['modify-modular'].refresh(modulObj))">
...
import { ModuleService } from "~/services/manager-service/module.service";
import { PageService } from "~/utils/page.service";
import DataForm from "~/components/common/data-form.vue";
import DataBox from "~/components/common/data-box.vue";
import addModular from "~/components/manager/system-manage/system-modular/add-modular.vue";
import modifyModular from "~/components/manager/system-manage/system-modular/modify-modular.vue";
...
export default class sysModuleDetail  extends Vue {
  ...
  refreshData() {
    this.moduleService.getModuleList(this.modularModel,this.pageService).subscribe(data => {
 ...
 /**
  ...
  goToPage(path,obj) {
    this.$router.push('/system-manage/modular-permission/'+obj);
  }
}
</script>
