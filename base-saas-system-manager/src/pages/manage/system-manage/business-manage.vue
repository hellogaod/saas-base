<template>
  ...
  <section class="page business-manage">
    ...
    <data-box :data="businessDataSet" :page="pageService" @onPageChange="refreshData" ref="data-box">
    ...
    <!--新增企业弹出框-->
    <el-dialog title="新增企业" :center="true" :visible.sync="dialog.add" width="500px" @close="$refs['add-business'].reset()">
      <add-business @close="dialog.add = false" ref="add-business" @refreshList="refreshData"></add-business>
    </el-dialog>

    <!--修改企业弹出框-->
    <el-dialog title="修改企业" :center="true" :visible.sync="dialog.modify" width="500px" @close="$refs['modify-business'].reset()" @open="$nextTick(()=>$refs['modify-business'].refresh(bussinessObj))">
      <modify-business ref="modify-business" @close="dialog.modify = false" @refreshList="refreshData"></modify-business>
    </el-dialog>

    <!--配置三方信息弹出框-->
    <el-dialog title="配置三方信息" :center="true" :visible.sync="dialog.config" width="500px" @close="$refs['config-business'].reset()" @open="$nextTick(()=>$refs['config-business'].refresh(bussinessObj))">
      <config-business ref="config-business" @close="dialog.config = false" @refreshList="refreshData"></config-business>
    </el-dialog>
...
import { EnterpiseService } from "~/services/manager-services/enterprise.service";
import { PageService } from "~/utils/page.service";
import DataForm from "~/components/common/data-form.vue";
import DataBox from "~/components/common/data-box.vue";
import AddBusiness from "~/components/manager/system-manage/business-manage/add-business.vue";
import ModifyBusiness from "~/components/manager/system-manage/business-manage/modify-business.vue";
import ConfigBusiness from "~/components/manager/system-manage/business-manage/config-business.vue";
...
export default class BussinessManage extends Vue {
  ...
  refreshData() {
    this.enterpiseService.getSysEnterpriseList(this.businessModel, this.pageService).subscribe(data => {
