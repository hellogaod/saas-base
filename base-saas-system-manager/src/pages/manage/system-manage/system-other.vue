<template>
  ...
  <section class="page system-other">
    ...
    <data-box :data="otherDataSet" :page="pageService" @onPageChange="refreshData" ref="data-box">
    ...
    <!--新增参数维护-->
    <el-dialog title="新增参数" :center="true" :visible.sync="dialog.add" width="1000px" @close="$refs['add-other'].reset()">
      <add-other @close="dialog.add = false" ref="add-other" @refreshList="refreshData"></add-other>
    </el-dialog>

    <!--修改参数-->
    <el-dialog title="修改参数" :center="true" :visible.sync="dialog.modify" width="1000px" @close="$refs['modify-other'].reset()" @open="$nextTick(()=>$refs['modify-other'].refresh(otherObj))">
...
import AddOther from "~/components/manager/system-manage/other-manage/add-other.vue";
import ModifyOther from "~/components/manager/system-manage/other-manage/modify-other.vue";
...
export default class OtherManage extends Vue {
  ...
  refreshData() {
    this.otherService.getOtherList(this.otherModel, this.pageService).subscribe(data => {
