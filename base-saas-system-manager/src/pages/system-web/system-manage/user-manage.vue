  ...
  <section class="page user-manage">
    <data-form :model="userModel" @onSearch="refreshData" :page="pageService">
      ...
      <template slot="default-button">
        <el-button @click="dialog.add = true" v-auth="'60959459-d53b-416b-bc5c-66b315cd44c7'">新增</el-button>
      </template>
    </data-form>
    <data-box :data="userDataSet"  @onPageChange="refreshData" :page="pageService" ref="data-box">
      <template slot="columns">
        ...
        <el-table-column prop="latestTime" label="最后一次登录时间" :min-width="$helper.getColumnWidth(4)">
          <template slot-scope="scope">
            <span>{{scope.row.latestTime|dateTimeFormat}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="lockTime" label="锁定时间" :min-width="$helper.getColumnWidth(4)">
          <template slot-scope="scope">
            <span>{{scope.row.lockTime|dateTimeFormat}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" :min-width="$helper.getColumnWidth(10)">
          <template slot-scope="scope">
            <el-button type="text" @click="dialog.modify=true,userObj=scope.row" v-auth="'884320d1-6ee9-47a1-bae0-b690d75ac0f8'">修改</el-button>
            <el-button type="text" @click="dialog.moduleRole=true,userObj=scope.row" v-auth="'334245e8-23c2-4251-b1be-0254d362685a'">角色分配</el-button>
            <el-button type="text" @click="enableClick(scope)" v-auth="'ff5e2fc2-c3a1-4309-9e04-6722f3a58043'" v-if="scope.row.status === 0">启用</el-button>
            <el-button type="text" @click="disableClick(scope)" v-auth="'ff5e2fc2-c3a1-4309-9e04-6722f3a58043'" v-if="scope.row.status === 1">停用</el-button>
            <el-button type="text" size="small" @click="resetPassword(scope)" v-auth="'e57878d7-0637-44c9-9ac8-29ada8610ebb'">重置密码</el-button>
          </template>
        </el-table-column>
      </template>
    </data-box>
    <!--新增用户-->
    <el-dialog title="新增" :center="true" :visible.sync="dialog.add" width="750px" @close="$refs['add-user'].reset()" @open="$nextTick(()=>$refs['add-user'].refresh())">
      <add-user @close="dialog.add = false" @refreshList="refreshData" ref="add-user"></add-user>
    </el-dialog>
    <!--修改用户-->
    <el-dialog title="修改" :center="true" :visible.sync="dialog.modify" width="750px" @close="$refs['modify-dialog'].reset()" @open="$nextTick(()=>$refs['modify-dialog'].refresh(userObj))">
      <modify-user @close="dialog.modify = false" @refreshList="refreshData" ref="modify-dialog"></modify-user>
    </el-dialog>
    <!--修改用户角色-->
    <el-dialog title="角色分配" :center="true" :visible.sync="dialog.moduleRole" width="900px" @open="$nextTick(()=>$refs['module-role'].refresh(userObj))">
...
import { sysUserService } from "~/services/systemweb-services/sysUser.service";
import { PageService } from "~/utils/page.service";
import { Action, State, Mutation } from "vuex-class";
// import { DepartmentInfoService } from "~/services/datasync-service/department-info.service";
import DataForm from "~/components/common/data-form.vue";
import DataBox from "~/components/common/data-box.vue";
import ModifyUser from "~/components/system-web/system-manage/user-manage/modify-user.vue";
import AddUser from "~/components/system-web/system-manage/user-manage/add-user.vue";
import ModuleRole from "~/components/system-web/system-manage/user-manage/module-role.vue";
...
export default class UserManage extends Vue {
  ...
  **/ 
  ...
  refreshData() {
    this.sysUserService.getUserList(this.userModel, this.pageService).subscribe(
      data => {
