  ...
  <section class="page role-manage">
    <data-form :model="roleModel" @onSearch="refreshData" :page="pageService">
      ...
      <template slot="default-button">
        <el-button @click="dialog.add = true" v-auth="'15252e7f-cd5a-4868-9db9-5936556e3b11'">新增角色</el-button>
      </template>
    </data-form>
    <data-box :data="roleDataSet" :page="pageService" @onPageChange="refreshData">
      <template slot="columns">
        ...
        <el-table-column label="操作" :min-width="$helper.getColumnWidth(5)">
          <template slot-scope="scope">
            <el-button v-auth="'f332e5e9-a52b-46e2-99e3-7f032a4c814e'" type="text" @click="enableClick(scope,0)" v-if="scope.row.status === 0">启用</el-button>
            <el-button v-auth="'f332e5e9-a52b-46e2-99e3-7f032a4c814e'" type="text" @click="disableClick(scope,0)" v-if="scope.row.status === 1">停用</el-button>
            <el-button v-auth="'6a526dd3-99c1-4271-9213-004c570b12e1'" type="text" @click="dialog.modify = true, roleObj = scope.row">修改</el-button>
            <el-button v-auth="'7a08d406-45db-4f4c-b78c-952780e6f087'" type="text" @click="dialog.module = true, roleObj = scope.row">权限分配</el-button>
          </template>
        </el-table-column>
      </template>
    </data-box>
    ...
    <!--修改角色弹出框-->
    <el-dialog title="修改角色" :center="true" :visible.sync="dialog.modify" width="500px" @open="$nextTick(()=>$refs['modify-role'].refresh(roleObj))">
      <modify-role ref="modify-role" @close="dialog.modify = false" @refreshList="refreshData"></modify-role>
    </el-dialog>

    <!--修改权限分配-->
    <el-dialog title="权限分配" :center="true" :visible.sync="dialog.module" width="300px" @open="$nextTick(()=>$refs['module-role'].refresh(roleObj))">
...
import { sysRoleService } from "~/services/systemweb-services/sysRole.service";
import { PageService } from "~/utils/page.service";
import DataForm from "~/components/common/data-form.vue";
import DataBox from "~/components/common/data-box.vue";
import AddRole from "~/components/system-web/system-manage/role-manage/add-role.vue";
import ModifyRole from "~/components/system-web/system-manage/role-manage/modify-role.vue";
import ModuleRole from "~/components/system-web/system-manage/role-manage/module-role.vue";
...
export default class RoleManage extends Vue {
   ...
   */
    ...
    refreshData() {
        this.sysRoleService.getRoleList(this.roleModel, this.pageService).subscribe(data => {
